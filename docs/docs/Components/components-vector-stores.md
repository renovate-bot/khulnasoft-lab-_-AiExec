...
# Vector store components in Aiexec
...
1. **Embedding Model**: Use your own embedding model by connecting an [Embeddings](/components-embedding-models) component in Aiexec.
...
This example uses `./chroma-db` to create a directory relative to where Aiexec is running.
...
| Name                         | Type          | Description                                      |
|------------------------------|---------------|--------------------------------------------------|
| collection_name               | String        | The name of the Chroma collection. Default: "aiexec". |
...
The **Local DB** component is Aiexec's enhanced version of Chroma DB.

The component adds a user-friendly interface with two modes (Ingest and Retrieve), automatic collection management, and built-in persistence in Aiexec's cache directory.

Local DB includes **Ingest** and **Retrieve** modes.

The **Ingest** mode works similarly to [ChromaDB](#chroma-db), and persists your database to the Aiexec cache directory. The Aiexec cache directory location is specified in `AIEXEC_CONFIG_DIR`. For more information, see [Environment variables](/environment-variables).
...
| Name                      | Type          | Description                                      |
|---------------------------|---------------|--------------------------------------------------|
| index_name                 | String        | The name of the FAISS index. Default: "aiexec_index". |
| persist_directory          | String        | Path to save the FAISS index. It is relative to where Aiexec is running. |
| search_query               | String        | The query to search for in the vector store.     |
| ingest_data                | Data          | The list of data to ingest into the vector store. |
