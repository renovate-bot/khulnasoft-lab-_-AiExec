...
In the Aiexec **Playground**, current sessions are listed on the left side of the pane.

Aiexec uses session IDs to track different chat interactions within flows. This allows multiple chat sessions to exist in a single flow. Messages are stored in the database with session IDs as a reference.
...
The `my_custom_session_value` value is used in components that accept it, and the stored messages from this flow are stored in `aiexec.db` with their respective `session_id` values.
...
Messages can be retrieved by `session_id` from the `/monitor` endpoint in the API. For more information, see the [API examples](https://docs.aiexec.org/api-reference-api-examples#get-messages).

For an example of session ID in action, see [Use Session IDs in Aiexec](https://www.youtube.com/watch?v=nJiF_eF21MY).