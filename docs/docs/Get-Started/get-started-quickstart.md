...
Get to know Aiexec by building an OpenAI-powered chatbot application. After you've constructed a chatbot, add Retrieval Augmented Generation (RAG) to chat with your own data.
...
1. From the Aiexec dashboard, click **New Flow**, and then select **Blank Flow**. A blank workspace opens where you can build your flow.
...
1. Add your credentials to the OpenAI component. The fastest way to complete these fields is with Aiexec’s [Global Variables](/configuration-global-variables).
...
You have successfully created a chatbot application using OpenAI in the Aiexec Workspace.

## Add vector RAG to your application

You created a chatbot application with Aiexec, but let's try an experiment.
...
10. If you don't have a collection, create a new one within the component.
	...
	2. Complete the **Name**, **Embedding generation method**, **Embedding model**, and **Dimensions** fields, and then click **Create**.
		...
		Your choice for the **Embedding generation method** and **Embedding model** depends on whether you want to use embeddings generated by a provider through Astra's vectorize service, or generated by a component in Aiexec.
		...
		* To use embeddings generated by a component in Aiexec, select **Bring your own** for both the **Embedding generation method** and **Embedding model** fields. In this starter project, the option for the embeddings method and model is the **OpenAI Embeddings** component connected to the **Astra DB** component.
...
If you used Aiexec's **Global Variables** feature, the RAG application flow components are already configured with the necessary credentials.
