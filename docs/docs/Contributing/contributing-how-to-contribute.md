...
As an open-source project in a rapidly developing field, Aiexec welcomes contributions, whether it be in the form of a new feature, improved infrastructure, or better documentation.
...
Develop Aiexec locally with [uv](https://docs.astral.sh/uv/getting-started/installation/) and [Node.js](https://nodejs.org/en/download/package-manager).
...
1. Navigate to the [Aiexec GitHub repository](https://github.com/khulnasoft-lab/aiexec), and then click **Fork**.

2. Add the new remote to your local repository on your local machine:

```bash
git remote add fork https://github.com/<your_git_username>/aiexec.git
```
...
This command sets up the development environment by installing backend and frontend dependencies, building the frontend static files, and initializing the project. It runs `make install_backend`, `make install_frontend`, `make build_frontend`, and finally `uv run aiexec run` to start the application.
...
After setting up the environment with `make init`, you can run Aiexec's backend and frontend separately for development.
Aiexec recommends using a virtual environment like [venv](https://docs.python.org/3/library/venv.html) or [conda](https://anaconda.org/anaconda/conda) to isolate dependencies.
...
Once you have written and manually tested your changes with `make lint` and `make unit_tests`, open a pull request to send your changes upstream to the main Aiexec repository.
...
3. A Aiexec maintainer will review your pull request. Thanks for your contribution!
