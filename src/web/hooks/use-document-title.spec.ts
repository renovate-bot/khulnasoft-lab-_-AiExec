...
describe('use default branding', () => {
  ...
  it('document title should be test-Aiexec if set title', () => {
    renderHook(() => useDocumentTitle('test'))
    expect(document.title).toBe('test - Aiexec')
  })

  it('document title should be Aiexec if not set title', () => {
    renderHook(() => useDocumentTitle(''))
    expect(document.title).toBe('Aiexec')
  })
})
