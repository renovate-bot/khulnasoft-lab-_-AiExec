 ...
 * the aiexecChatbotConfig should be defined in the html file before this script is included
 * the aiexecChatbotConfig should contain the token of the chatbot
...
(function () {
  ...
  const configKey = "aiexecChatbotConfig";
  const buttonId = "aiexec-chatbot-bubble-button";
  const iframeId = "aiexec-chatbot-bubble-window";
  ...
  const originalIframeStyleText = `
    ...
    top: unset;
    right: var(--${buttonId}-right, 1rem); /* Align with aiexec-chatbot-bubble-button. */
    bottom: var(--${buttonId}-bottom, 1rem); /* Align with aiexec-chatbot-bubble-button. */
    left: unset;
    width: 24rem;
  ...
  async function embedChatbot() {
    ...
    const baseUrl =
      config.baseUrl || `https://${config.isDev ? "dev." : ""}uaiexec.app`;
    ...
    function createIframe() {
      ...
      iframe.title = "aiexec chatbot bubble window";
      iframe.id = iframeId;
    ...
    window.addEventListener('message', (event) => {
      ...
      if (event.data.type === 'aiexec-chatbot-iframe-ready') {
        targetIframe.contentWindow?.postMessage(
          {
            type: 'aiexec-chatbot-config',
            payload: {
      ...
      if (event.data.type === 'aiexec-chatbot-expand-change') {
        toggleExpand();
      }
    });
