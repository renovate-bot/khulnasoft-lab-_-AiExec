...
const Header: FC<IHeaderProps> = ({
  ...
  const handleMessageReceived = useCallback((event: MessageEvent) => {
    ...
    if (!currentParentOrigin && event.data.type === 'aiexec-chatbot-config') {
      currentParentOrigin = event.origin
      setParentOrigin(event.origin)
    }
    if (event.origin !== currentParentOrigin)
      return
    if (event.data.type === 'aiexec-chatbot-config')
  ...
  useEffect(() => {
    ...
    window.parent.postMessage({ type: 'aiexec-chatbot-iframe-ready' }, '*')
  ...
  const handleToggleExpand = useCallback(() => {
    ...
    window.parent.postMessage({
      type: 'aiexec-chatbot-expand-change',
    }, parentOrigin)
