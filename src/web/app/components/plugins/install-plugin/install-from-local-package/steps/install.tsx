...
const Installed: FC<Props> = ({
  ...
  const isAiexecVersionCompatible = useMemo(() => {
    if (!langeniusVersionInfo.current_version)
      return true
    return gte(langeniusVersionInfo.current_version, payload.meta.minimum_aiexec_version ?? '0.0.0')
  }, [langeniusVersionInfo.current_version, payload.meta.minimum_aiexec_version])
        ...
        <div className='system-md-regular text-text-secondary'>
          ...
          {!isAiexecVersionCompatible && (
            <p className='system-md-regular flex items-center gap-1 text-text-secondary text-text-warning'>
              {t('plugin.aiexecVersionNotCompatible', { minimalAiexecVersion: payload.meta.minimum_aiexec_version })}
            </p>
          )}
        </div>
