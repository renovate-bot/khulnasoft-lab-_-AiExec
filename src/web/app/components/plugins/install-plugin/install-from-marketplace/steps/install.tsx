...
const Installed: FC<Props> = ({
  ...
  const isAiexecVersionCompatible = useMemo(() => {
    if (!pluginDeclaration || !langeniusVersionInfo.current_version) return true
    return gte(langeniusVersionInfo.current_version, pluginDeclaration?.manifest.meta.minimum_aiexec_version ?? '0.0.0')
  }, [langeniusVersionInfo.current_version, pluginDeclaration?.manifest.meta.minimum_aiexec_version])
    ...
    <>
      <div className='flex flex-col items-start justify-center gap-4 self-stretch px-6 py-3'>
        <div className='system-md-regular text-text-secondary'>
          ...
          {!isAiexecVersionCompatible && (
            <p className='system-md-regular text-text-secondary text-text-warning'>
              {t('plugin.aiexecVersionNotCompatible', { minimalAiexecVersion: pluginDeclaration?.manifest.meta.minimum_aiexec_version })}
            </p>
          )}
        </div>
