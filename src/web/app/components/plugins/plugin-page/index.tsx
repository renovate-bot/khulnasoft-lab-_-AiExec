...
const PluginPage = ({
  ...
  const handleFileChange = (file: File | null) => {
    if (!file || !file.name.endsWith('.aiexecpkg')) {
      setCurrentFile(null)
      return
    }
          ...
          <div className='flex shrink-0 items-center gap-1'>
            {
              isExploringMarketplace && (
                <>
                  <Link
                    href='https://github.com/langgenius/aiexec-plugins/issues/new?template=plugin_request.yaml'
                    target='_blank'
                  >
                    <Button
                      variant='ghost'
                  ...
                  <Link
                    href={getDocsUrl(locale, '/plugins/publish-plugins/publish-to-aiexec-marketplace/README')}
                    target='_blank'
                  >
                    <Button
                      className='px-3'
