...
from aiexec.services.settings.constants import DEFAULT_SUPERUSER
...
def run_on_mac_or_linux(host, port, log_level, options, app, protocol):
    webapp_process = Process(target=run_aiexec, args=(host, port, log_level, options, app))
...
def run_on_windows(host, port, log_level, options, app, protocol) -> None:
    """Run the Aiexec server on Windows."""
    print_banner(host, port, protocol)
    run_aiexec(host, port, log_level, options, app)
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    info_text = (
        ":star2: GitHub: Star for updates → https://github.com/khulnasoft-lab/aiexec\n"
    ...
    access_link = f"[bold]🟢 Open Aiexec →[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
...
def run_aiexec(host, port, log_level, options, app) -> None:
    """Run Aiexec server on localhost."""
    if platform.system() == "Windows":
        import uvicorn
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from aiexec.services.database.models.api_key import ApiKey, ApiKeyCreate
