...
class AiexecSetup(Base):
    __tablename__ = "aiexec_setups"
    __table_args__ = (db.PrimaryKeyConstraint("version", name="aiexec_setup_pkey"),)
...
class App(Base):
    ...
    def api_base_url(self):
        return (aiexec_config.SERVICE_API_URL or request.host_url.rstrip("/")) + "/v1"
...
class Conversation(Base):
    ...
    def inputs(self):
        ...
        for key, value in inputs.items():
            ...
            if isinstance(value, dict) and value.get("aiexec_model_identity") == FILE_MODEL_IDENTITY:
                ...
                isinstance(item, dict) and item.get("aiexec_model_identity") == FILE_MODEL_IDENTITY for item in value
            ):
                inputs[key] = []
                for item in value:
...
class Site(Base):
    ...
    def app_base_url(self):
        return aiexec_config.APP_WEB_URL or request.url_root.rstrip("/")
