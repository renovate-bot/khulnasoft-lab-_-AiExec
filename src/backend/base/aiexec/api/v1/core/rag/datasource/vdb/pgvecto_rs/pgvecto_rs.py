    ...
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> PGVectoRS:
        ...
        return PGVectoRS(
            ...
            config=PgvectoRSConfig(
                host=aiexec_config.PGVECTO_RS_HOST or "localhost",
                port=aiexec_config.PGVECTO_RS_PORT or 5432,
                user=aiexec_config.PGVECTO_RS_USER or "postgres",
                password=aiexec_config.PGVECTO_RS_PASSWORD or "",
                database=aiexec_config.PGVECTO_RS_DATABASE or "postgres",
            ),
            dim=dim,
        )
