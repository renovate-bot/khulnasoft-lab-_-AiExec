        ...
        with redis_client.lock(lock_name, timeout=20):
            ...
            if not self._has_collection():
                ...
                self._client.create_collection(
                    ...
                    fields=fields,
                    description="Collection For Aiexec",
                )
...
class VikingDBVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> VikingDBVector:
        ...
        if aiexec_config.VIKINGDB_ACCESS_KEY is None:
            raise ValueError("VIKINGDB_ACCESS_KEY should not be None")
        if aiexec_config.VIKINGDB_SECRET_KEY is None:
            raise ValueError("VIKINGDB_SECRET_KEY should not be None")
        if aiexec_config.VIKINGDB_HOST is None:
            raise ValueError("VIKINGDB_HOST should not be None")
        if aiexec_config.VIKINGDB_REGION is None:
            raise ValueError("VIKINGDB_REGION should not be None")
        if aiexec_config.VIKINGDB_SCHEME is None:
            raise ValueError("VIKINGDB_SCHEME should not be None")
        return VikingDBVector(
            collection_name=collection_name,
            group_id=dataset.id,
            config=VikingDBConfig(
                access_key=aiexec_config.VIKINGDB_ACCESS_KEY,
                secret_key=aiexec_config.VIKINGDB_SECRET_KEY,
                host=aiexec_config.VIKINGDB_HOST,
                region=aiexec_config.VIKINGDB_REGION,
                scheme=aiexec_config.VIKINGDB_SCHEME,
                connection_timeout=aiexec_config.VIKINGDB_CONNECTION_TIMEOUT,
                socket_timeout=aiexec_config.VIKINGDB_SOCKET_TIMEOUT,
            ),
        )
