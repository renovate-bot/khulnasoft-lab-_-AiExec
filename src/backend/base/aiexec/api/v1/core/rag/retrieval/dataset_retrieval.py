...
class DatasetRetrieval:
    ...
    def retrieve(
        ...
        aiexec_documents = [item for item in all_documents if item.provider == "aiexec"]
        ...
        # deal with aiexec documents
        if aiexec_documents:
            records = RetrievalService.format_retrieval_documents(aiexec_documents)
            if records:
    ...
    def _on_retrieval_end(
        ...
        aiexec_documents = [document for document in documents if document.provider == "aiexec"]
        for document in aiexec_documents:
            if document.metadata is not None:
                dataset_document = (
                    db.session.query(DatasetDocument)
    ...
    def _fetch_model_config(
        ...
        if provider_model.status == ModelStatus.NO_CONFIGURE:
            ...
            raise ValueError(f"Aiexec Hosted OpenAI {model_name} currently not support.")
