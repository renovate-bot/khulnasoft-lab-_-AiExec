...
class AnalyticdbVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> AnalyticdbVector:
        ...
        if aiexec_config.ANALYTICDB_HOST is None:
            # implemented through OpenAPI
            apiConfig = AnalyticdbVectorOpenAPIConfig(
                access_key_id=aiexec_config.ANALYTICDB_KEY_ID or "",
                access_key_secret=aiexec_config.ANALYTICDB_KEY_SECRET or "",
                region_id=aiexec_config.ANALYTICDB_REGION_ID or "",
                instance_id=aiexec_config.ANALYTICDB_INSTANCE_ID or "",
                account=aiexec_config.ANALYTICDB_ACCOUNT or "",
                account_password=aiexec_config.ANALYTICDB_PASSWORD or "",
                namespace=aiexec_config.ANALYTICDB_NAMESPACE or "",
                namespace_password=aiexec_config.ANALYTICDB_NAMESPACE_PASSWORD,
            )
            sqlConfig = None
        else:
            # implemented through sql
            sqlConfig = AnalyticdbVectorBySqlConfig(
                host=aiexec_config.ANALYTICDB_HOST,
                port=aiexec_config.ANALYTICDB_PORT,
                account=aiexec_config.ANALYTICDB_ACCOUNT or "",
                account_password=aiexec_config.ANALYTICDB_PASSWORD or "",
                min_connection=aiexec_config.ANALYTICDB_MIN_CONNECTION,
                max_connection=aiexec_config.ANALYTICDB_MAX_CONNECTION,
                namespace=aiexec_config.ANALYTICDB_NAMESPACE or "",
            )
            apiConfig = None
