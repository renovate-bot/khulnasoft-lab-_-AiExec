...
class LindormVectorStore(BaseVector):
    ...
    def create_collection(self, dimension: int, **kwargs):
        ...
        with redis_client.lock(lock_name, timeout=20):
            ...
            method_name = kwargs.pop("method_name", aiexec_config.DEFAULT_INDEX_TYPE)
            space_type = kwargs.pop("space_type", aiexec_config.DEFAULT_DISTANCE_TYPE)
...
class LindormVectorStoreFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> LindormVectorStore:
        lindorm_config = LindormVectorStoreConfig(
            hosts=aiexec_config.LINDORM_URL or "",
            username=aiexec_config.LINDORM_USERNAME,
            password=aiexec_config.LINDORM_PASSWORD,
            using_ugc=aiexec_config.USING_UGC_INDEX,
            request_timeout=aiexec_config.LINDORM_QUERY_TIMEOUT,
        )
        using_ugc = aiexec_config.USING_UGC_INDEX
        ...
        if dataset.index_struct:
            ...
            index_type = aiexec_config.DEFAULT_INDEX_TYPE
            distance_type = aiexec_config.DEFAULT_DISTANCE_TYPE
