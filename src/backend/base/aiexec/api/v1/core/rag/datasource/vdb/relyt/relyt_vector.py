...
class RelytVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> RelytVector:
        ...
        return RelytVector(
            ...
            config=RelytConfig(
                host=aiexec_config.RELYT_HOST or "localhost",
                port=aiexec_config.RELYT_PORT,
                user=aiexec_config.RELYT_USER or "",
                password=aiexec_config.RELYT_PASSWORD or "",
                database=aiexec_config.RELYT_DATABASE or "default",
            ),
            group_id=dataset.id,
        )
