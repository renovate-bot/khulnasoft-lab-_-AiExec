...
class ToolManager:
        ...
        with db.session.no_autoflush:
            if "builtin" in filters:
                ...
                for provider in builtin_providers:
                    ...
                    if is_filtered(
                        include_set=cast(set[str], aiexec_config.POSITION_TOOL_INCLUDES_SET),
                        exclude_set=cast(set[str], aiexec_config.POSITION_TOOL_EXCLUDES_SET),
                        data=provider,
    ...
    def generate_builtin_tool_icon_url(cls, provider_id: str) -> str:
        return str(
            URL(aiexec_config.CONSOLE_API_URL or "/")
            / "console"
            / "api"
            / "workspaces"
            / "current"
