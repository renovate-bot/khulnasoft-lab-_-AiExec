    ...
    def _extract_files(self, outputs: dict) -> tuple[dict, list[File]]:
        ...
        for key, value in outputs.items():
            if isinstance(value, list):
                for item in value:
                    if isinstance(item, dict) and item.get("aiexec_model_identity") == FILE_MODEL_IDENTITY:
                        ...
                        files.append(file)
            elif isinstance(value, dict) and value.get("aiexec_model_identity") == FILE_MODEL_IDENTITY:
