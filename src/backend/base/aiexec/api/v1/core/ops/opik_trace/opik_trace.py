...
def prepare_opik_uuid(user_datetime: Optional[datetime], user_uuid: Optional[str]):
    """Opik needs UUIDv7 while Aiexec uses UUIDv4 for identifier of most
...
class OpikDataTrace(BaseTraceInstance):
    ...
    def workflow_trace(self, trace_info: WorkflowTraceInfo):
        aiexec_trace_id = trace_info.workflow_run_id
        opik_trace_id = prepare_opik_uuid(trace_info.start_time, aiexec_trace_id)
        ...
        if trace_info.message_id:
            aiexec_trace_id = trace_info.message_id
            opik_trace_id = prepare_opik_uuid(trace_info.start_time, aiexec_trace_id)
