...
class CodeNode(BaseNode[CodeNodeData]):
    ...
    def _check_string(self, value: str | None, variable: str) -> str | None:
        ...
        if len(value) > aiexec_config.CODE_MAX_STRING_LENGTH:
            raise OutputValidationError(
                f"The length of output variable `{variable}` must be"
                f" less than {aiexec_config.CODE_MAX_STRING_LENGTH} characters"
            )
    ...
    def _check_number(self, value: int | float | None, variable: str) -> int | float | None:
        ...
        if value > aiexec_config.CODE_MAX_NUMBER or value < aiexec_config.CODE_MIN_NUMBER:
            raise OutputValidationError(
                f"Output variable `{variable}` is out of range,"
                f" it must be between {aiexec_config.CODE_MIN_NUMBER} and {aiexec_config.CODE_MAX_NUMBER}."
            )

        if isinstance(value, float):
            # raise error if precision is too high
            if len(str(value).split(".")[1]) > aiexec_config.CODE_MAX_PRECISION:
                raise OutputValidationError(
                    f"Output variable `{variable}` has too high precision,"
                    f" it must be less than {aiexec_config.CODE_MAX_PRECISION} digits."
                )
    ...
    def _transform_result(
        ...
        if depth > aiexec_config.CODE_MAX_DEPTH:
            raise DepthLimitError(f"Depth limit {aiexec_config.CODE_MAX_DEPTH} reached, object too deep.")
        ...
        for output_name, output_config in output_schema.items():
            ...
            if output_config.type == "object":
                ...
                if not isinstance(result[output_name], list):
                    ...
                    if len(result[output_name]) > aiexec_config.CODE_MAX_NUMBER_ARRAY_LENGTH:
                        raise OutputValidationError(
                            f"The length of output variable `{prefix}{dot}{output_name}` must be"
                            f" less than {aiexec_config.CODE_MAX_NUMBER_ARRAY_LENGTH} elements."
                        )
                ...
                if not isinstance(result[output_name], list):
                    ...
                    if len(result[output_name]) > aiexec_config.CODE_MAX_STRING_ARRAY_LENGTH:
                        raise OutputValidationError(
                            f"The length of output variable `{prefix}{dot}{output_name}` must be"
                            f" less than {aiexec_config.CODE_MAX_STRING_ARRAY_LENGTH} elements."
                        )
                ...
                if not isinstance(result[output_name], list):
                    ...
                    if len(result[output_name]) > aiexec_config.CODE_MAX_OBJECT_ARRAY_LENGTH:
                        raise OutputValidationError(
                            f"The length of output variable `{prefix}{dot}{output_name}` must be"
                            f" less than {aiexec_config.CODE_MAX_OBJECT_ARRAY_LENGTH} elements."
                        )
