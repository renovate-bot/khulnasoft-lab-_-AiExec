...
When a model invocation error occurs, it should be mapped to the runtime's specified `InvokeError` type, enabling Aiexec to handle different errors appropriately.
...
For interface method details, see: [Interfaces](./interfaces.md). For specific implementations, refer to: [llm.py](https://github.com/langgenius/aiexec-runtime/blob/main/lib/model_providers/anthropic/llm/llm.py).
