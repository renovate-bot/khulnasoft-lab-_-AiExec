...
- Invocation Error Mapping Table
  ...
  When there is an exception in model invocation, it needs to be mapped to the `InvokeError` type specified by Runtime. This facilitates Aiexec's ability to handle different errors with appropriate follow-up actions.
...
For details on the interface methods, see: [Interfaces](interfaces.md). For specific implementations, refer to: [llm.py](https://github.com/langgenius/aiexec-runtime/blob/main/lib/model_providers/anthropic/llm/llm.py).
