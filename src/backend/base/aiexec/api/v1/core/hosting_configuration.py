...
class HostingConfiguration:
    ...
    def init_app(self, app: Flask) -> None:
        if aiexec_config.EDITION != "CLOUD":
            return
    ...
    def init_azure_openai() -> HostingProvider:
        ...
        if aiexec_config.HOSTED_AZURE_OPENAI_ENABLED:
            credentials = {
                "openai_api_key": aiexec_config.HOSTED_AZURE_OPENAI_API_KEY,
                "openai_api_base": aiexec_config.HOSTED_AZURE_OPENAI_API_BASE,
                "base_model_name": "gpt-35-turbo",
            }

            quotas: list[HostingQuota] = []
            hosted_quota_limit = aiexec_config.HOSTED_AZURE_OPENAI_QUOTA_LIMIT
    ...
    def init_openai(self) -> HostingProvider:
        ...
        if aiexec_config.HOSTED_OPENAI_TRIAL_ENABLED:
            hosted_quota_limit = aiexec_config.HOSTED_OPENAI_QUOTA_LIMIT
        ...
        if aiexec_config.HOSTED_OPENAI_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "openai_api_key": aiexec_config.HOSTED_OPENAI_API_KEY,
            }

            if aiexec_config.HOSTED_OPENAI_API_BASE:
                credentials["openai_api_base"] = aiexec_config.HOSTED_OPENAI_API_BASE

            if aiexec_config.HOSTED_OPENAI_API_ORGANIZATION:
                credentials["openai_organization"] = aiexec_config.HOSTED_OPENAI_API_ORGANIZATION
    ...
    def init_anthropic() -> HostingProvider:
        ...
        if aiexec_config.HOSTED_ANTHROPIC_TRIAL_ENABLED:
            hosted_quota_limit = aiexec_config.HOSTED_ANTHROPIC_QUOTA_LIMIT
            trial_quota = TrialHostingQuota(quota_limit=hosted_quota_limit)
            quotas.append(trial_quota)

        if aiexec_config.HOSTED_ANTHROPIC_PAID_ENABLED:
            paid_quota = PaidHostingQuota()
            quotas.append(paid_quota)

        if len(quotas) > 0:
            credentials = {
                "anthropic_api_key": aiexec_config.HOSTED_ANTHROPIC_API_KEY,
            }

            if aiexec_config.HOSTED_ANTHROPIC_API_BASE:
                credentials["anthropic_api_url"] = aiexec_config.HOSTED_ANTHROPIC_API_BASE
    ...
    def init_minimax() -> HostingProvider:
        quota_unit = QuotaUnit.TOKENS
        if aiexec_config.HOSTED_MINIMAX_ENABLED:
    ...
    def init_spark() -> HostingProvider:
        quota_unit = QuotaUnit.TOKENS
        if aiexec_config.HOSTED_SPARK_ENABLED:
            quotas: list[HostingQuota] = [FreeHostingQuota()]
    ...
    def init_zhipuai() -> HostingProvider:
        quota_unit = QuotaUnit.TOKENS
        if aiexec_config.HOSTED_ZHIPUAI_ENABLED:
            quotas: list[HostingQuota] = [FreeHostingQuota()]
    ...
    def init_moderation_config() -> HostedModerationConfig:
        if aiexec_config.HOSTED_MODERATION_ENABLED and aiexec_config.HOSTED_MODERATION_PROVIDERS:
            providers = aiexec_config.HOSTED_MODERATION_PROVIDERS.split(",")
            hosted_providers = []
    ...
    def parse_restrict_models_from_env(env_var: str) -> list[RestrictModel]:
        models_str = aiexec_config.model_dump().get(env_var)
