...
from configs.app_config import AiexecConfig


def test_aiexec_config(monkeypatch):
    ...
    monkeypatch.setenv("DB_DATABASE", "aiexec")
...
def test_flask_configs(monkeypatch):
    ...
    flask_app.config.from_mapping(AiexecConfig().model_dump())  # pyright: ignore
    ...
    assert config["SQLALCHEMY_DATABASE_URI"] == "postgresql://postgres:postgres@localhost:5432/aiexec"
