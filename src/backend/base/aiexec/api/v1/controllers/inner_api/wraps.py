...
def enterprise_inner_api_only(view):
    ...
    def decorated(*args, **kwargs):
        if not aiexec_config.INNER_API:
            abort(404)

        # get header 'X-Inner-Api-Key'
        inner_api_key = request.headers.get("X-Inner-Api-Key")
        if not inner_api_key or inner_api_key != aiexec_config.INNER_API_KEY:
            abort(401)
...
def plugin_inner_api_only(view):
    @wraps(view)
    def decorated(*args, **kwargs):
        if not aiexec_config.PLUGIN_DAEMON_KEY:
            abort(404)

        # get header 'X-Inner-Api-Key'
        inner_api_key = request.headers.get("X-Inner-Api-Key")
        if not inner_api_key or inner_api_key != aiexec_config.INNER_API_KEY_FOR_PLUGIN:
            abort(404)
