...
def init_app(app: AiexecApp):
    ...
    def init_flask_instrumentor(app: AiexecApp):
        meter = get_meter("http_metrics", version=aiexec_config.CURRENT_VERSION)
    ...
    class ExceptionLoggingHandler(logging.Handler):
        ...
        def emit(self, record: logging.LogRecord):
            try:
                if record.exc_info:
                    tracer = get_tracer_provider().get_tracer("aiexec.exception.logging")
    ...
    resource = Resource(
        attributes={
            ResourceAttributes.SERVICE_NAME: aiexec_config.APPLICATION_NAME,
            ResourceAttributes.SERVICE_VERSION: f"aiexec-{aiexec_config.CURRENT_VERSION}-{aiexec_config.COMMIT_SHA}",
            ResourceAttributes.PROCESS_PID: os.getpid(),
            ResourceAttributes.DEPLOYMENT_ENVIRONMENT: f"{aiexec_config.DEPLOY_ENV}-{aiexec_config.EDITION}",
            ResourceAttributes.HOST_NAME: socket.gethostname(),
            ResourceAttributes.HOST_ARCH: platform.machine(),
            "custom.deployment.git_commit": aiexec_config.COMMIT_SHA,
    ...
    sampler = ParentBasedTraceIdRatio(aiexec_config.OTEL_SAMPLING_RATE)
    ...
    protocol = (aiexec_config.OTEL_EXPORTER_OTLP_PROTOCOL or "").lower()
    if aiexec_config.OTEL_EXPORTER_TYPE == "otlp":
        if protocol == "grpc":
            exporter = GRPCSpanExporter(
                endpoint=aiexec_config.OTLP_BASE_ENDPOINT,
                # Header field names must consist of lowercase letters, check RFC7540
                headers=(("authorization", f"Bearer {aiexec_config.OTLP_API_KEY}"),),
                insecure=True,
            )
            ...
            exporter = HTTPSpanExporter(
                endpoint=aiexec_config.OTLP_BASE_ENDPOINT + "/v1/traces",
                headers={"Authorization": f"Bearer {aiexec_config.OTLP_API_KEY}"},
            )
            metric_exporter = HTTPMetricExporter(
                endpoint=aiexec_config.OTLP_BASE_ENDPOINT + "/v1/metrics",
                headers={"Authorization": f"Bearer {aiexec_config.OTLP_API_KEY}"},
            )
    else:
    ...
    provider.add_span_processor(
        BatchSpanProcessor(
            exporter,
            max_queue_size=aiexec_config.OTEL_MAX_QUEUE_SIZE,
            schedule_delay_millis=aiexec_config.OTEL_BATCH_EXPORT_SCHEDULE_DELAY,
            max_export_batch_size=aiexec_config.OTEL_MAX_EXPORT_BATCH_SIZE,
            export_timeout_millis=aiexec_config.OTEL_BATCH_EXPORT_TIMEOUT,
        )
    )
    reader = PeriodicExportingMetricReader(
        metric_exporter,
        export_interval_millis=aiexec_config.OTEL_METRIC_EXPORT_INTERVAL,
        export_timeout_millis=aiexec_config.OTEL_METRIC_EXPORT_TIMEOUT,
    )
...
def is_enabled():
    return aiexec_config.ENABLE_OTEL
