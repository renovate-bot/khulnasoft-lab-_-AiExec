...
class SupabaseStorage(BaseStorage):
    ...
    def __init__(self):
        super().__init__()
        if aiexec_config.SUPABASE_URL is None:
            raise ValueError("SUPABASE_URL is not set")
        if aiexec_config.SUPABASE_API_KEY is None:
            raise ValueError("SUPABASE_API_KEY is not set")
        if aiexec_config.SUPABASE_BUCKET_NAME is None:
            raise ValueError("SUPABASE_BUCKET_NAME is not set")

        self.bucket_name = aiexec_config.SUPABASE_BUCKET_NAME
        self.client = Client(supabase_url=aiexec_config.SUPABASE_URL, supabase_key=aiexec_config.SUPABASE_API_KEY)
        self.create_bucket(id=aiexec_config.SUPABASE_BUCKET_NAME, bucket_name=aiexec_config.SUPABASE_BUCKET_NAME)
