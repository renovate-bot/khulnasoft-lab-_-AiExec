...
class BaiduObsStorage(BaseStorage):
    ...
    def __init__(self):
        super().__init__()
        self.bucket_name = aiexec_config.BAIDU_OBS_BUCKET_NAME
        client_config = BceClientConfiguration(
            credentials=BceCredentials(
                access_key_id=aiexec_config.BAIDU_OBS_ACCESS_KEY,
                secret_access_key=aiexec_config.BAIDU_OBS_SECRET_KEY,
            ),
            endpoint=aiexec_config.BAIDU_OBS_ENDPOINT,
        )
