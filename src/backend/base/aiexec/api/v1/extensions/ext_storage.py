...
class Storage:
    def init_app(self, app: Flask):
        storage_factory = self.get_storage_factory(aiexec_config.STORAGE_TYPE)
    ...
    def get_storage_factory(storage_type: str) -> Callable[[], BaseStorage]:
        match storage_type:
            ...
            case StorageType.OPENDAL:
                ...
                return lambda: OpenDALStorage(aiexec_config.OPENDAL_SCHEME)
            case StorageType.LOCAL:
                from extensions.storage.opendal_storage import OpenDALStorage

                return lambda: OpenDALStorage(scheme="fs", root=aiexec_config.STORAGE_LOCAL_PATH)
