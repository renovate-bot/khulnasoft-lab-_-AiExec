...
def init_app(app: AiexecApp):
    ...
    def after_request(response):
        ...
        response.headers.add("X-Version", aiexec_config.CURRENT_VERSION)
        response.headers.add("X-Env", aiexec_config.DEPLOY_ENV)
        return response

    @app.route("/health")
    def health():
        return Response(
            json.dumps({"pid": os.getpid(), "status": "ok", "version": aiexec_config.CURRENT_VERSION}),
            status=200,
