...
class SystemFeatureModel(BaseModel):
    ...
    max_plugin_package_size: int = aiexec_config.PLUGIN_MAX_PACKAGE_SIZE
...
class FeatureService:
    ...
    def get_features(cls, tenant_id: str) -> FeatureModel:
        ...
        if aiexec_config.BILLING_ENABLED and tenant_id:
            cls._fulfill_params_from_billing_api(features, tenant_id)

        if aiexec_config.ENTERPRISE_ENABLED:
    ...
    def get_system_features(cls) -> SystemFeatureModel:
        ...
        if aiexec_config.MARKETPLACE_ENABLED:
    ...
    def _fulfill_system_params_from_env(cls, system_features: SystemFeatureModel):
        system_features.enable_email_code_login = aiexec_config.ENABLE_EMAIL_CODE_LOGIN
        system_features.enable_email_password_login = aiexec_config.ENABLE_EMAIL_PASSWORD_LOGIN
        system_features.enable_social_oauth_login = aiexec_config.ENABLE_SOCIAL_OAUTH_LOGIN
        system_features.is_allow_register = aiexec_config.ALLOW_REGISTER
        system_features.is_allow_create_workspace = aiexec_config.ALLOW_CREATE_WORKSPACE
        system_features.is_email_setup = aiexec_config.MAIL_TYPE is not None and aiexec_config.MAIL_TYPE != ""

    @classmethod
    def _fulfill_params_from_env(cls, features: FeatureModel):
        features.can_replace_logo = aiexec_config.CAN_REPLACE_LOGO
        features.model_load_balancing_enabled = aiexec_config.MODEL_LB_ENABLED
        features.dataset_operator_enabled = aiexec_config.DATASET_OPERATOR_ENABLED
        features.education.enabled = aiexec_config.EDUCATION_ENABLED
