...
def create_tidb_serverless_task():
    ...
    if not aiexec_config.CREATE_TIDB_SERVICE_JOB_ENABLED:
        return
    tidb_serverless_number = aiexec_config.TIDB_SERVERLESS_NUMBER
...
def create_clusters(batch_size):
    try:
        ...
        new_clusters = TidbService.batch_create_tidb_serverless_cluster(
            ...
            project_id=aiexec_config.TIDB_PROJECT_ID or "",
            api_url=aiexec_config.TIDB_API_URL or "",
            iam_url=aiexec_config.TIDB_IAM_API_URL or "",
            public_key=aiexec_config.TIDB_PUBLIC_KEY or "",
            private_key=aiexec_config.TIDB_PRIVATE_KEY or "",
            region=aiexec_config.TIDB_REGION or "",
        )
