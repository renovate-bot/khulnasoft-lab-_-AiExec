...
from aiexec.custom.directory_reader.utils import (
...
from aiexec.custom.schema import MissingDefault
from aiexec.field_typing.range_spec import RangeSpec
from aiexec.helpers.custom import format_type
from aiexec.schema import dotdict
from aiexec.template.field.base import Input
from aiexec.template.frontend_node.custom_components import ComponentFrontendNode, CustomComponentFrontendNode
from aiexec.type_extraction.type_extraction import extract_inner_type
from aiexec.utils import validate
from aiexec.utils.util import get_base_classes
...
async def get_single_component_dict(component_type: str, component_name: str, components_paths: list[str]):
    ...
    for base_path in components_paths:
        ...
        if module_path.exists():
            ...
            module_name = f"aiexec.components.{component_type}.{component_name}"
            try:
...
async def load_custom_component(component_name: str, components_paths: list[str]):
    ...
    """
    from aiexec.interface.custom_component import get_custom_component_from_name
                ...
                for category_dir in base_path.iterdir():
                    if category_dir.is_dir():
                        ...
                        if component_file.exists():
                            # Try to import the module
                            module_name = f"aiexec.components.{category_dir.name}.{component_name}"
                            try:
                                import importlib.util
