...
def send_invite_member_mail_task(language: str, to: str, token: str, inviter_name: str, workspace_name: str):
    ...
    try:
        url = f"{aiexec_config.CONSOLE_WEB_URL}/activate?token={token}"
        if language == "zh-Hans":
            ...
            if system_features.branding.enabled:
                ...
                mail.send(to=to, subject="立即加入 Aiexec 工作空间", html=html_content)
        else:
            ...
            if system_features.branding.enabled:
                ...
                mail.send(to=to, subject="Join Aiexec Workspace Now", html=html_content)
