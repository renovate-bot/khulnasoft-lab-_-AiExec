...
from models.model import AiexecSetup
...
REFRESH_TOKEN_EXPIRY = timedelta(days=aiexec_config.REFRESH_TOKEN_EXPIRE_DAYS)
...
class AccountService:
    ...
    def get_account_jwt_token(account: Account) -> str:
        exp_dt = datetime.now(UTC) + timedelta(minutes=aiexec_config.ACCESS_TOKEN_EXPIRE_MINUTES)
        exp = int(exp_dt.timestamp())
        payload = {
            "user_id": account.id,
            "exp": exp,
            "iss": aiexec_config.EDITION,
    ...
    def create_account(
        ...
        if aiexec_config.BILLING_ENABLED and BillingService.is_email_in_freeze(email):
    ...
    def add_login_error_rate_limit(email: str) -> None:
        ...
        if count is None:
            count = 0
        count = int(count) + 1
        redis_client.setex(key, aiexec_config.LOGIN_LOCKOUT_DURATION, count)
    ...
    def add_forgot_password_error_rate_limit(email: str) -> None:
        ...
        if count is None:
            count = 0
        count = int(count) + 1
        redis_client.setex(key, aiexec_config.FORGOT_PASSWORD_LOCKOUT_DURATION, count)
    ...
    def is_email_send_ip_limit(ip_address: str):
        ...
        if current_minute_count > aiexec_config.EMAIL_SEND_IP_LIMIT_PER_MINUTE:
...
class RegisterService:
    ...
    def setup(cls, email: str, name: str, password: str, ip_address: str) -> None:
        ...
        try:
            ...
            aiexec_setup = AiexecSetup(version=aiexec_config.CURRENT_VERSION)
            db.session.add(aiexec_setup)
            db.session.commit()
        except Exception as e:
            db.session.query(AiexecSetup).delete()
    ...
    def invite_new_member(
        ...
        send_invite_member_mail_task.delay(
            ...
            to=email,
            token=token,
            inviter_name=inviter.name if inviter else "Aiexec",
    ...
    def generate_invite_token(cls, tenant: Tenant, account: Account) -> str:
        ...
        expiry_hours = aiexec_config.INVITE_EXPIRY_HOURS
