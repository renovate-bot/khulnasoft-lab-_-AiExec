...
code_execution_endpoint_url = URL(str(aiexec_config.CODE_EXECUTION_ENDPOINT))
...
class CodeExecutor:
    ...
    def execute_code(cls, language: CodeLanguage, preload: str, code: str) -> str:
        ...
        headers = {"X-Api-Key": aiexec_config.CODE_EXECUTION_API_KEY}
        ...
        try:
            response = post(
                ...
                headers=headers,
                timeout=Timeout(
                    connect=aiexec_config.CODE_EXECUTION_CONNECT_TIMEOUT,
                    read=aiexec_config.CODE_EXECUTION_READ_TIMEOUT,
                    write=aiexec_config.CODE_EXECUTION_WRITE_TIMEOUT,
                    pool=None,
                ),
            )
