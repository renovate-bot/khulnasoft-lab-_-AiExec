...
def get_signed_file_url(upload_file_id: str) -> str:
    url = f"{aiexec_config.FILES_URL}/files/{upload_file_id}/file-preview"

    timestamp = str(int(time.time()))
    nonce = os.urandom(16).hex()
    key = aiexec_config.SECRET_KEY.encode()
...
def get_signed_file_url_for_plugin(filename: str, mimetype: str, tenant_id: str, user_id: str) -> str:
    url = f"{aiexec_config.FILES_URL}/files/upload/for-plugin"
...
def verify_plugin_file_signature(
    *, filename: str, mimetype: str, tenant_id: str, user_id: str | None, timestamp: str, nonce: str, sign: str
    ...
    secret_key = aiexec_config.SECRET_KEY.encode()
