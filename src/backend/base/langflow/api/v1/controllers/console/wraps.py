...
def only_edition_self_hosted(view):
    @wraps(view)
    def decorated(*args, **kwargs):
        if aiexec_config.EDITION != "SELF_HOSTED":
            abort(404)
            ...
            if features.billing.enabled:
                if resource == "add_segment":
                    if features.billing.subscription.plan == "sandbox":
                        abort(
                            403,
                            "To unlock this feature and elevate your Aiexec experience, please upgrade to a paid plan.",
                        )
                else:
                    return view(*args, **kwargs)
...
def setup_required(view):
    ...
    def decorated(*args, **kwargs):
        # check setup
        if (
            aiexec_config.EDITION == "SELF_HOSTED"
            and os.environ.get("INIT_PASSWORD")
            and not db.session.query(AiexecSetup).first()
        ):
            raise NotInitValidateError()
        elif aiexec_config.EDITION == "SELF_HOSTED" and not db.session.query(AiexecSetup).first():
            raise NotSetupError()
