...
A Aiexec App Service-API Client, using for build a webapp by request Service-API

## Usage

First, install `aiexec-client` python sdk package:

```
pip install aiexec-client
```
...
```python
from aiexec_client import CompletionClient
...
```python
import json
from aiexec_client import ChatClient
...
```python
from aiexec_client import AiexecClient

api_key = "your_api_key"

# Initialize Client
aiexec_client = AiexecClient(api_key)
...
with open(file_path, "rb") as file:
    ...
    response = aiexec_client.file_upload("user_id", files)
