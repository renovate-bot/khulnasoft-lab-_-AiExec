...
class AiexecClient:
    def __init__(self, api_key, base_url: str = "https://api.aiexec.ai/v1"):
...
class CompletionClient(AiexecClient):
...
class WorkflowClient(AiexecClient):
    def run(
...
class KnowledgeBaseClient(AiexecClient):
    def __init__(
        self,
        api_key,
        base_url: str = "https://api.aiexec.ai/v1",
        ...
        Args:
            api_key (str): API key of Aiexec.
            base_url (str, optional): Base URL of Aiexec API. Defaults to 'https://api.aiexec.ai/v1'.
