...
from aiexec_client.client import (
    ChatClient,
    CompletionClient,
    AiexecClient,
    KnowledgeBaseClient,
...
API_BASE_URL = os.environ.get("API_BASE_URL", "https://api.aiexec.ai/v1")
...
class TestAiexecClient(unittest.TestCase):
    def setUp(self):
        self.aiexec_client = AiexecClient(API_KEY)

    def test_message_feedback(self):
        response = self.aiexec_client.message_feedback(
            "your_message_id", "like", "test_user"
        )
        self.assertIn("success", response.text)

    def test_get_application_parameters(self):
        response = self.aiexec_client.get_application_parameters("test_user")
    ...
    def test_file_upload(self):
        ...
        with open(file_path, "rb") as file:
            ...
            response = self.aiexec_client.file_upload("test_user", files)
